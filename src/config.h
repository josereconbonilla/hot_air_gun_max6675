...
class HOTGUN_CFG : public CONFIG {
  public:
    ...
    uint8_t dsplBright(void) {
      return Config.dspl_bright;
    }
    void saveBright(uint8_t br) {
      Config.dspl_bright = br;
    }
  ...
  private:
    uint16_t t_tip[3];
    const uint16_t def_tip[3] = {200, 400, 600}; // Default values of internal sensor readings at reference temperatures
    const uint16_t min_temp = 25;
    const uint16_t max_temp = 600;
    const uint16_t def_temp = 350; // Default preset temperature
