...
class HOTGUN_HW {
  public:
    HOTGUN_HW( uint8_t HG_pwr_pin);
    void init(void);
    uint8_t appliedPower(void) {
      return actual_power;
    }
    uint16_t tempDispersion(void) {
      return h_temp.dispersion();
    }
    bool areExternalInterrupts(void) {
      return millis() - last_period < period * 15;
    }
    bool syncCB(void); // Return true at the end of the power period

    float readTemp_Max6675(void);

    int16_t readTemperatureMax6675(void);
  ...
  private:
    ...
    unsigned long readTempMax6675 = millis();
};
...
class HOTGUN : public HOTGUN_HW, public PID {
  public:
    ...
    HOTGUN(uint8_t HG_pwr_pin) :
      HOTGUN_HW(HG_pwr_pin), h_power(hot_gun_hist_length) { }
    void init(void);
    bool isOn(void) {
    ...
    uint16_t presetFan(void) {
      return fan_speed;
    }
    uint16_t averageTemp(void) {
      return h_temp.read();
    }
    uint16_t averagePower(void) {
      return h_power.read();
    }
    uint8_t getMaxFixedPower(void) {
      return max_fix_power;
    }
    bool isCold(void) {
      return h_temp.read() < temp_gun_cold;
    }
    bool isFanWorking(void) {
      return (fanSpeed() >= min_fan_speed);
    }
    uint16_t maxFanSpeed(void) {
      return max_fan_speed;
    }
    uint16_t pwrDispersion(void) {
      return d_power.read();
    }
    void setTemp(uint16_t temp) {
      //temp_set  = constrain(temp, 0, temp_max);
      temp_set  = temp;
    }
    ...
    void setFan(uint16_t fan) {
      fan_speed = constrain(fan, min_fan_speed, max_fan_speed);
    }
  ...
  private:
    ...
    const uint16_t temp_gun_cold = 80; // The temperature of the cold Hot Air Gun
