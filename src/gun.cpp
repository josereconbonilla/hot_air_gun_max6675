...
HOTGUN_HW::HOTGUN_HW(uint8_t HG_pwr_pin) {
  gun_pin = HG_pwr_pin;
}
...
float HOTGUN_HW::readTemp_Max6675(void) {
  int a = 0;
  digitalWrite(10, LOW);
  ...
  else return ((a >> 3) * 0.25);
}

int16_t HOTGUN_HW::readTemperatureMax6675(void) {
  if (millis() > readTempMax6675) {
    readTempMax6675 = millis() + SENSOR_SAMPLING_TIME;
    uint16_t t = readTemp_Max6675();
    return t; // Update hot gun temperature
  }
}
...
void HOTGUN::keepTemp(void) {
  ...
  uint16_t r = readTemperatureMax6675();
  h_temp.update(r); // Update hot gun temperature
